...
require_once($_SERVER['PHP_INCLUDE_HOME'] . "/lp2/GI_BrowseCountry.php");
require_once('DB.php');
...
class Article {
  ...
  function Article() {
   ...
   //else if (strpos($contentObject->get(), '<!-- insert citation rps -->')){
    ...
    //create country browse html 
    if (strpos($this->_content, '<!--insert country browse -->')) {
    $countryObject = new BrowseCountry();	
    $this->_content = preg_replace('#<!-- db continent object here -->#', $countryObject->get_toplevel(), $this->_content);
    $this->_content = preg_replace('#<!--insert country browse -->#', $countryObject->get(), $this->_content);
    }
  }
  function get() {
