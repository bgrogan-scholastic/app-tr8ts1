...
if(__DEBUG__) {
	require_once($_SERVER['PHP_INCLUDE_HOME'] . 'common/utils/GI_MicroTimer.php');
}
...
class AtlasBase extends GI_Base {
	 ...
	 * @param      $_db
	 * @access protected 12/05/2006 SJF: This should not be public.
	 */
	protected $_db;
	 ...
	 * @param      $_params
	 * @access protected 12/05/2006 SJF.  If accessed from outside the class, it can be made public.
	 */
	protected $_params;
	 ...
	 * mostly the database
	 *
	 * @access public - 12/05/2006 was documented as private but is public and has to be as
	 * it is called from code that does not exist in this class.
	 * 
	 * 12/05/06 Consider converting this to a proper PHP 5 destructor.  We don't do it
	 * hear since it is called from a standalone function defined further down in this file.
	 * 
	 */
	...
	public function getNavBar($template) {
		...
		if($_SERVER[GI_AUTH_PCODE] == GO_PRODUCT_ID and $this->_params[OPTION] != GLOBALDIST) {
	 ...
	 * @access public
	 */
	public function getMapGraphic($subTemplate) {
		$retval = NULL;
 ...
 *  12/05/2006 SJF
 *
 *  The following function is not defined in a class.  It stands alone.
 */
...
class AtlasLatsLongs extends AtlasBase {
	...
	public function & getJavascript() {
		...
		$retval = file_get_contents($_SERVER["JAVASCRIPT_INCLUDE_HOME"] . "go2/atlas/distance.js");
 ...
 * @return    : ?
 * 
 * 12/05/2006 PHP 5 conversion - SJF
 * 
 */
...
class AtlasPOI extends AtlasBase {
	...
	function __construct() {
		...
		if(!DB::isError($poiAsset)) {
			...
			$this->_poiFilepath = GI_hashFilename($this->_poiFilename, $_SERVER['DOCUMENT_ROOT'] . "/media");
			...
			$this->_captionFilepath = GI_hashFilename($this->_captionFilename, $_SERVER['DOCUMENT_ROOT'] . "/cache");
	...
	public function getImage($template) {
		...
		// build the key array
		$keys = array();
		$keys["urlfilepath"] = substr($this->_poiFilepath, strlen($_SERVER['DOCUMENT_ROOT']));
...
/*
 * 12/05/2006 SJF- The following function stands alone...
...
class ErrorManager extends GI_ErrorManager {
	 ...
	 * @access public SJF: I question whether or not this should be public.  I assume so since this method is 
	 * not called by this class.
	 */
	public function setBuildresult($value) {
	 ...
	 * not called by this class.
	 */
	public function setFatalerror($value) {
...
/*
 * SJF 12/05/2006 The following is a standalone function 
 */
