<?php
	...
	if (!DB::isError($db)) {
        ...
        $qString .= " where cb.pid='" . $pair_cat . "' and cb.cid = m.slp_id and m.type='0cp' and m.volume in (" . $volumesString . ")";
		...
		if (!DB::isError($result)) {
    		...
    		while ($dbrow = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            	...
            	if (!GI_ManifestMiner::iserror($pairMiner)) {
                    ...
                    echo "<link><![CDATA[/p/" . $pair_cat . "/" . $dbrow['SLPID'] . "]]></link>\n";
                	...
                	if (!GI_ManifestMiner::iserror($storyMiner))
	                {
                        ...
                        echo "<cover><![CDATA[/csimage?product_id=allflix&id=" . $cover['slp_id'] . "&skipdb=y]]></cover>\n";
    	            }
                	...
                	if (!GI_ManifestMiner::iserror($bookMiner))
	                {
                        ...
                        echo "<jacket><![CDATA[/csimage?product_id=allflix&id=" . $jacket['slp_id'] . "&skipdb=y]]></jacket>\n";
                        #echo "\n<!--\n";
                        #print_r($bookMiner);
                        #echo "\n-->\n";
                    }
