...
require($_SERVER["PHP_INCLUDE_HOME"]."locker1/data_classes/locker_shard.php");
require_once('DB.php');
...
$grandparentid = '800078146';
		//Check to see if user exists

		$statement = $this->_iauthDBConn->prepare("CALL ".GET_SAMCUSTOMER."(?)");
		$statement->bind_param("s", $guid);
		$statement->execute();
		$samcustrow = $this->getresult($statement);
		
		if(empty($samcustrow[0]['customer_id']))
		{		
			//Get Grandparent info
			$statement = $this->_iauthDBConn->prepare("CALL ".GET_SAMPARENTDETAILS."(?)");
			$statement->bind_param("s", $grandparentid);
			$statement->execute();
			$grandparent_row = $this->getresult($statement);
			
			//Check for dupes in iauth.uid table					
			$statement = $this->_iauthDBConn->prepare("SELECT ".GET_USERNAMECOUNT."(?, ?) as usernamecount");
			$statement->bind_param("si", $guid, $grandparent_row[0]['source_id']);
			$statement->execute();	
			$dupe_row = $this->getresult($statement);			
			...
			$statement = $this->_iauthDBConn->prepare("SELECT ".INSERT_CUSTOMERIDGEN."() as customer_id");
			$statement->execute();	
			$newcustomer_row = $this->getresult($statement);			

			//add iauth to customer_id
			$newcustomer_row[0]['customer_id'] = "iauth".$newcustomer_row[0]['customer_id'];
			
			//INSERT INTO iauth.customer
			$variable = $guid." @ ".$grandparent_row[0]['name'];
			$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_CUSTOMER."(?, ?, ?)");
			$statement->bind_param("sss", $newcustomer_row[0]['customer_id'], $grandparent_row[0]['source_id'], $variable);
			$statement->execute();	

			//insertsamcustomer_000001
			$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_SAMCUSTOMER."(?, ?)");
			$statement->bind_param("ss", $guid,$newcustomer_row[0]['customer_id']);
			$statement->execute();

			//Which shard to put the user
			
			$statement = $this->_iauthDBConn->prepare("CALL ".GET_STATESHARD."(?)");
			$statement->bind_param("s", $statecode);
			$statement->execute();
			$shard_row = $this->getresult($statement);
			$shardtotal = 0;
			
			if(empty($shard_row[0]['shard_id']))
			{	

				//Create a customerid for state parent
				//INSERT INTO iauth.customeridgen
				$statement = $this->_iauthDBConn->prepare("SELECT ".INSERT_CUSTOMERIDGEN."() as customer_id");
				$statement->execute();	
				$statecustomer_row = $this->getresult($statement);				
	
				//add iauth to customer_id
				$statecustomer_row[0]['customer_id'] = "iauth".$statecustomer_row[0]['customer_id'];
				
				//INSERT INTO iauth.customer
				$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_CUSTOMER."(?, ?, ?)");
				$variable = $statecode." @ ".$grandparent_row[0]['name'];
				$statement->bind_param("sss", $statecustomer_row[0]['customer_id'], $grandparent_row[0]['source_id'], $variable);
				$statement->execute();	
				//Insert into customer for state parent
				
				//Get the next shard
				$statement = $this->_iauthDBConn->prepare("CALL ".GET_NEXTSHARD."()");
				$statement->execute();	
				$shard_row = $this->getresult($statement);				
				
				//Add parent to shard.
				$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_PARENTIDLOOKUP."(?, ?)");
				$statement->bind_param("si", $statecustomer_row[0]['customer_id'], $shard_row[0]['shard_id']);
				$statement->execute();		

				//Update the shard counts
				$statement = $this->_iauthDBConn->prepare("CALL ".UPDATE_SHARDCOUNT."(?, ?)");
				$shardtotal = $shard_row[0]['total']+1;
				$statement->bind_param("ii", $shard_row[0]['shard_id'], $shardtotal);
				$statement->execute();	

				//INSERT INTO stateid_lookup	
				$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_STATEIDGEN."(?, ?)");	
				$statement->bind_param("ss", $statecode, $statecustomer_row[0]['customer_id']);
				$statement->execute();				

				//CREATE STATE AU
				//INSERT INTO iauth.auidgen
				$statement = $this->_iauthDBConn->prepare("SELECT ".INSERT_AUIDGEN."() as au_id");
				$statement->execute();	
				$stateauid_row = $this->getresult($statement);

				$mainvar = 'MAIN';
				$onevar =  '1';
				//INSERT INTO iauth.au
				$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_AU."(?, ?, ?, ?, ?, ?, ?)");
				$statement->bind_param("iisssss", $stateauid_row[0]['au_id'], $grandparent_row[0]['source_id'], $grandparentid, $grandparentid, $statecustomer_row[0]['customer_id'], $mainvar,$onevar);
				$statement->execute();	

				//CREATE PREFS		
				//Find the au for the grandparent!
$au_product='eto';
				$statement = $this->_iauthDBConn->prepare("CALL ".GET_AUBYPROD."(?,?)");
				$statement->bind_param('ss', $grandparentid, $au_product);
				$statement->execute();
				$grandparent_au_row = $this->getresult($statement); 									

				//GET PREFS from parent Look in auth.au_pref
				$statement = $this->_iauthDBConn->prepare("CALL ".GET_PARENTPREFS."(?)");
				$statement->bind_param('i', $grandparent_au_row[0]['au_id']);
				$statement->execute();
				$grandparent_prefs_row = $this->getresult($statement); 				

				//Loop through prefs and insert
				for($i=0; $i < count($grandparent_prefs_row); $i++)
				{
					echo "DEBUG STATE PREF::".$grandparent_prefs_row[$i]['pref_code']."<br>";
					//INSERT INTO iauth.au_pref
					$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_AUPREF."(?, ?, ?)");
					$statement->bind_param('iss', $stateauid_row[0]['au_id'], $grandparent_prefs_row[$i]['pref_code'], $grandparent_prefs_row[$i]['pref_value']);
					$statement->execute();
				}

				//CREATE PRODUCTS
				//GET PRODS from parent Look in auth.au_prod
				$statement = $this->_iauthDBConn->prepare("CALL ".GET_PARENTPRODS."(?)");
				$statement->bind_param('i', $grandparent_au_row[0]['au_id']);
				$statement->execute();
				$grandparent_prods_row = $this->getresult($statement); 	
							
				//Loop through prefs and insert

				for($i=0; $i < count($grandparent_prods_row); $i++)
				{
					//INSERT INTO iauth.au_prod
					$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_AUPROD."(?, ?)");
					$statement->bind_param('is', $stateauid_row[0]['au_id'], $grandparent_prods_row[$i]['product_code']);
					$statement->execute();
				}			
			}
			else 
			{
				...
				//GET $stateauid_row[0]['au_id'] AND $statecustomer_row[0]['customer_id'] info
				$statement = $this->_iauthDBConn->prepare("CALL ".GET_STATEAU."(?)");
				$statement->bind_param('s', $statecode);
				$statement->execute();
				$state_row = $this->getresult($statement);
				$stateauid_row[0]['au_id'] = $state_row[0]['au_id'];
				$statecustomer_row[0]['customer_id'] = $state_row[0]['customer_id'];
				
				//GET SHARD TOTALS
				$statement = $this->_iauthDBConn->prepare("CALL ".GET_SHARDCOUNTS."(?)");
				$statement->bind_param("i", $shard_row[0]['shard_id']);
				$statement->execute();
				$shard_row = $this->getresult($statement);
				$shardtotal = $shard_row[0]['total'];
			}
		
			//INSERT INTO iauth.profileidgen
			
			$statement = $this->_iauthDBConn->prepare("SELECT ".INSERT_PROFILEIDGEN."() as profile_id");
			$statement->execute();	
			$profile_row = $this->getresult($statement);			

			//UPDATE SHARD COUNT
			$statement = $this->_iauthDBConn->prepare("CALL ".UPDATE_SHARDCOUNT."(?, ?)");
			$shardtotal = $shardtotal+1;
			$statement->bind_param("ii", $shard_row[0]['shard_id'], $shardtotal);
			$statement->execute();	

			//SETUP LOCKER DB CONNECTION!!(the state customer id is the parent)
			$this->createDbConnection($profile_row[0]['profile_id'].'.'.$statecustomer_row[0]['customer_id']);

			//INSERT INTO iauth.auth_profile
			$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_AUTHPROFILE."(?, ?, ?)");
			$statement->bind_param("isi", $grandparent_row[0]['source_id'], $newcustomer_row[0]['customer_id'], $profile_row[0]['profile_id']);
			$statement->execute();				

			//INSERT INTO locker tables;	
			$statement = $this->_lockerdbConn->prepare("SELECT ".CREATE_USER."(?, ?, ?, ?, ?, ?) as assignmentid");
...
$productid = 'eto';
			...
			$statement->bind_param("isisis", $profile_row[0]['profile_id'], $userdata['name']['first'], $securityquestionid, $securityanswer, $profiletype,$productid);
			$statement->execute();	
			$user_row = $this->getresult($statement); 
			
			$profilevalue = '';
			$profilevalue .= $userdata['email'].'|'.$userdata['course'].'||';
			$this->updateprofiledetails($profile_row[0]['profile_id'], $productid, $user_row[0]['assignmentid'], $profilevalue, $userdata['lexile_score']);
			
			//CREATE AU
			//INSERT INTO iauth.auidgen
			$statement = $this->_iauthDBConn->prepare("SELECT ".INSERT_AUIDGEN."() as au_id");
			$statement->execute();	
			$auid_row = $this->getresult($statement); 			

			$mainvar = 'MAIN';
			$onevar =  '1';
			//INSERT INTO iauth.au
			$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_AU."(?, ?, ?, ?, ?, ?, ?)");
			$statement->bind_param("iisssss", $auid_row[0]['au_id'], $grandparent_row[0]['source_id'], $statecustomer_row[0]['customer_id'], $statecustomer_row[0]['customer_id'], $newcustomer_row[0]['customer_id'], $mainvar,$onevar);
			$statement->execute();	

			//INSERT INTO iauth.uid OR MAYBE AN UPDATE!!!
			$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_UID."(?, ?, ?)");
			$statement->bind_param("sis", $guid, $grandparent_row[0]['source_id'], $guid );
			$statement->execute();				
			...
			$param8 = 'NULL';
			$param9='';
			$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_AUUID."(?, ?, ?, ?, ?, ?, ?, ?)");
			$statement->bind_param('isiiiiss', $auid_row[0]['au_id'], $guid, $grandparent_row[0]['source_id'],$param5,$param6,$param7,$param8,$param9);
			$statement->execute();				

			//CREATE PREFS
			//GET PREFS from parent Look in auth.au_pref
			$statement = $this->_iauthDBConn->prepare("CALL ".GET_IAUTHPARENTPREFS."(?)");
			$statement->bind_param('s', $stateauid_row[0]['au_id']);
			$statement->execute();
			$parent_prefs_row = $this->getresult($statement); 				
			...
			for($i=0; $i < count($parent_prefs_row); $i++)
			{
				//INSERT INTO iauth.au_pref
				$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_AUPREF."(?, ?, ?)");
				$statement->bind_param('iss', $auid_row[0]['au_id'], $parent_prefs_row[$i]['pref_code'], $parent_prefs_row[$i]['pref_value']);
				$statement->execute();
			...
			//CREATE PRODUCTS
			//GET PRODS from parent Look in auth.au_prod
			$statement = $this->_iauthDBConn->prepare("CALL ".GET_IAUTHPARENTPRODS."(?)");
			$statement->bind_param('i', $stateauid_row[0]['au_id']);
			$statement->execute();
			$parent_prods_row = $this->getresult($statement); 	
			...
			for($i=0; $i < count($parent_prods_row); $i++)
			{
				//INSERT INTO iauth.au_prod
				$statement = $this->_iauthDBConn->prepare("CALL ".INSERT_AUPROD."(?, ?)");
				$statement->bind_param('is', $auid_row[0]['au_id'], $parent_prods_row[$i]['product_code']);
				$statement->execute();
			}
		...
		else 
		{
			...
			$statement = $this->_iauthDBConn->prepare("CALL ".GET_PROFILEID."(?)");
			$statement->bind_param("s", $guid);
			$statement->execute();	
			$user_row = $this->getresult($statement);				
			$profileid = $user_row[0]['profile_id'].".".$user_row[0]['parent_id'];

			$securityquestionid = 5;
			$securityanswer = 'sam';
$productid = 'eto';
			$currassignment = $this->getcurrentassignment($profileid,$productid);
			$currassignment = $currassignment->getassignmentid();
			...
			$this->updateprofile($profileid, $guid,$guid, $userdata['name']['first'], $securityquestionid, $securityanswer, $profiletype);
			$profilevalue = '';
			$profilevalue .= $userdata['email'].'|'.$userdata['course'].'||';
			$this->updateprofiledetails($profileid, $productid, $currassignment, $profilevalue, $userdata['lexile_score']);
		}
	...
	*/
	...
	{			
		...
		{	
			...
			if(empty($productid))
			{
				$productid = 'wegotnothing';
			}
			
			// 'test'.$user_row[0]['profile_id'];
			$getprofcookiestatement->bind_param("is", $user_row[0]['profile_id'],$productid);
			...
			list($emailaddress, $gradelist, $interestlist, $marketingpreflist) = explode("|", $profcookie_row[0]['profilevalue']);
			//Change , to . for the cookie. Otherwise wont work.
			$gradelist = str_replace(',',".", $gradelist);
			$gradeORcourse = '|grade>'.$gradelist;	
			if($productid == 'eto')
			{
				$gradeORcourse = '|course>'.$gradelist;	
			}
			...
			$cookievalues = 'prof_values_'.$productid;
			$cookiearray[$cookievalues] = 'currassgn>'.$profcookie_row[0]['currentassignmentid'].'|readlvl>'.$profcookie_row[0]['reading_level'].'|outlineid>'.$outlinerow[0]['outlineid'].'|bibliographyid>'.$bibrow[0]['bibliographyid'].$gradeORcourse;
	...
	* getprofileid
	*
	* Returns a profileid for a given userid.
	*
	* @author  	John Palmer
	* @access  	public 
	* 
...
//		return "<$parent_auid>1.".$parent_row[0]['recipient_id'];
	...
	*/
	...
	{			
		$parentid = '';
		list($tempvar, $parentid) = split('[.]', $profileid);
		$profileid = $this->createDbConnection($profileid);
		...
		$shardstatement->bind_param("s", $parentid);
		$shardstatement->execute();	
		$shardrow = $this->getresult($shardstatement);
		...
		$profileobj = new Locker_Profile($profileid,'','',$profilerow[0]['productid'], $profilerow[0]['nickname'], $profilerow[0]['securityquestionid'], $profilerow[0]['securityanswer'], $profilerow[0]['lastlogindate'],
					...
					$profilerow[0]['currentassignmentid'],$gradearray,$interestarray,$marketingprefarray,$parentid, $shardrow[0]['shard_id']);
		return $profileobj;
	}	
	...
	* getuserlist
	*
	* This function returns an array of Locker_IAuthProfile object for a given parentid. 
	* 
	* 
	* @author  John Palmer
	* @access  public 
	...
	*/
	...
	{			
		$userliststatement = $this->_iauthDBConn->prepare("CALL ".GET_ALLUSERS."()");
		$userliststatement->execute();	
		$userlist = $this->getresult($userliststatement);			
		...
		{	
			//Create Profile Object
			$profileobj = new Locker_IAuthProfile('','','',$userlist[$i]['profile_id'].'.'.$parentid,$userlist[$i]['user_id'],$userlist[$i]['recipient_id'],$userlist[$i]['user_password'],$userlist[$i]['parent_id']);
			$retval[] = $profileobj;	
		}
	...
	* searchuserlist
	*
	* This function returns an array of Locker_IAuthProfile object for a given user name string. 
	* 
	* 
	* @author  John Palmer
	* @access  public 
	...
	*/
	...
	{	
		...
		$etodb = DB::connect($_SERVER['ETO_DB_CONNECT_STRING']);
		if (DB::isError($etodb)) {
			print "connection to {$_SERVER['ETO_DB_CONNECT_STRING']} failed ";
		}			
			...
			for ($i = 0; $i<count($savedassetlinkrow); $i++)
			{
				...
				//if the asset is an image...
					...
					if($productid=="go"){
						...
						$uid = $mainExt['uid'];
						$id = $mainExt['id'];
						$newtitle = urlencode($mainExt['title']);	
						$fext = $mainExt['fext'];									
					...
					//if the product is not go...		
						...
						$id = $assetArray[0]['id'];
						$newtitle = urlencode($savedassetlinkrow[$i]['title']);
					...
					$url = str_replace("##UID##", $savedassetlinkrow[$i]['uid'], $url);
					
					//if the uid is blank then remove it from the url
					if($savedassetlinkrow[$i]['uid'] == ""){
					...
					if($type=="0uv"){
						...
						if($productid=='eto'){
							$manresult = $etodb->query($mansql)->fetchrow();
						...
						if($category == "xs02_anchor" || $category == "xt02_anchor"){
...
//SHOULD WE HAVE A LOCK?!?!
			...
			$createuserstatement->bind_param("isisis", $profile_row[0]['profile_id'], $nickname, $securityquestionid, $securityanswer, $profiletype,$_SERVER['LOCKER_PCODE']);
			...
			$getparentprefsstatement->bind_param('i', $parent_auid);
	...
	*/
	...
	{			
		...
		$insertsavedassetstatement->bind_param('issssss', $profileid, $assignmentid, $assetid, $sourceproductid, $type, $title, $uid);
	...
	*/
	...
	{			
		$retval = array();
		$statement = $this->_iauthDBConn->prepare("CALL ".GET_SHARDLIST."()");
		$statement->execute();	
		$row = $this->getresult($statement);	
		for ($i = 0; $i<count($row); $i++)
		{
			$shardobj = new Locker_Shard($row[$i]['shard_id'],$row[$i]['db_hostname'],$row[$i]['db_username'],$row[$i]['db_password'],$row[$i]['db_name'],$row[$i]['db_port'],'','','','','','','','');
			$retval[] = $shardobj;
		}				
		return $retval;
	}
	...
	*/
	...
	{			
		$retval = array();
		$statement = $this->_iauthDBConn->prepare("CALL ".GET_SHARDCOUNTSLIST."()");
		$statement->execute();	
		$row = $this->getresult($statement);	
		for ($i = 0; $i<count($row); $i++)
		{
			$shardobj = new Locker_Shard($row[$i]['shard_id'],'','','','','',$row[$i]['total'],'','','','','','','');
			$retval[] = $shardobj;
		}				
		return $retval;
	}
	...
	*/
	...
	{			
		$retval = array();
		$statement = $this->_iauthDBConn->prepare("CALL ".GET_PARENTTOSHARDCOUNTSLIST."()");
		$statement->execute();	
		$row = $this->getresult($statement);	
		for ($i = 0; $i<count($row); $i++)
		{
	...
	*/
	...
	{			
		$retval = array();
		//Get list of users ordered by shard_id
		$statement = $this->_iauthDBConn->prepare("CALL ".GET_SHARDUSERLIST."()");
		$statement->execute();	
		$shardrow = $this->getresult($statement);
	
		$currentShard = $shardrow[0]['shard_id'];
		$profileid = $shardrow[0]['profile_id'].'.'.$shardrow[0]['parent_id'];
		$queryStart = "Select count(*) as total from $elementtype where profileid IN (";
		$queryEnd = '';
		//print_r($shardrow);
		for ($i = 0; $i<=count($shardrow); $i++)
		{
			//Check to see if we have a new shard or if we are at the end!
			if($currentShard != $shardrow[$i]['shard_id'] || $i==count($shardrow))
			{
				//If yes!!! Run query
				$queryEnd = substr($queryEnd, 0, -1); 
				$queryEnd .= ")";
				//Get list of users ordered by shard_id
				$query = $queryStart.$queryEnd;
				
				//Close the old connectiont
		    	if($this->_lockerdbConn != null)
		    	{
					$this->_lockerdbConn->close();
		    	}
				...
				//Open new connection because we are changing shards
				$this->createDbConnection($profileid);
				
				$profstatement = $this->_lockerdbConn->prepare($query);
				$profstatement->execute();	
				$profrow = $this->getresult($profstatement);
				...
				{
				    case 'assignment':
						$shardobj = new Locker_Shard($currentShard,'','','','','','','',$profrow[0]['total'],'','','','','');        
					break;
				    case 'groups':
						$shardobj = new Locker_Shard($currentShard,'','','','','','','','',$profrow[0]['total'],'','','','');        
				    break;
				...
				//Set Current Shard to NEW Shard.  ONLY DO THIS IF WE ARE NOT AT THE END!
				if($i!=count($shardrow))
				{
					$currentShard = $shardrow[$i]['shard_id'];
					$profileid = $shardrow[$i]['profile_id'].'.'.$shardrow[$i]['parent_id'];
				}
				$queryEnd = '';
			}
			else 
			{
				//If no!!! Add profileid to query	
				$queryEnd .= "'".$shardrow[$i]['profile_id']."',";
			}			
		}				
		return $retval;
	}		
	...
	*/
	...
	{			
		...
		$shardHostName = $shardrow[0]['db_hostname'];
		$shardUserName = $shardrow[0]['db_username'];
		$shardPassword = $shardrow[0]['db_password'];
		$shardDB = $shardrow[0]['db_name'];
		$shardPort = $shardrow[0]['db_port'];
		...
		for ($i = 0; $i<=count($shardrow); $i++)
		{
			...
			{
				//If yes!!! Run query
				$queryStart = "Select count(*) as total from assignment where profileid IN (";
				$queryEnd = substr($queryEnd, 0, -1); 
				$queryEnd .= ")";
				...
				$profstatement = $this->_lockerdbConn->prepare($query);
				$profstatement->execute();	
				$assignmentrow = $this->getresult($profstatement);

				$queryStart = "Select count(*) as total from groups where profileid IN (";
				$query = $queryStart.$queryEnd;
				$profstatement = $this->_lockerdbConn->prepare($query);
				$profstatement->execute();	
				$groupsrow = $this->getresult($profstatement);

				$queryStart = "Select count(*) as total from notecard where profileid IN (";
				$query = $queryStart.$queryEnd;
				$profstatement = $this->_lockerdbConn->prepare($query);
				$profstatement->execute();	
				$notecardrow = $this->getresult($profstatement);				

				$queryStart = "Select count(*) as total from citation where profileid IN (";
				$query = $queryStart.$queryEnd;
				$profstatement = $this->_lockerdbConn->prepare($query);
				$profstatement->execute();	
				$citationrow = $this->getresult($profstatement);

				$queryStart = "Select count(*) as total from task where profileid IN (";
				$query = $queryStart.$queryEnd;
				$profstatement = $this->_lockerdbConn->prepare($query);
				$profstatement->execute();	
				$taskrow = $this->getresult($profstatement);

				$queryStart = "Select count(*) as total from savedasset where profileid IN (";
				$query = $queryStart.$queryEnd;
				$profstatement = $this->_lockerdbConn->prepare($query);
				$profstatement->execute();	
				$savedassetrow = $this->getresult($profstatement);

				$profstatement = $this->_iauthDBConn->prepare("CALL ".GET_SHARDCOUNTS."(?)");
				$profstatement->bind_param('i', $currentShard);
				$profstatement->execute();
				$userrow = $this->getresult($profstatement); 		
		
				$profstatement = $this->_iauthDBConn->prepare("CALL ".GET_PARENTTOSHARDCOUNTS."(?)");
				$profstatement->bind_param('i', $currentShard);
				$profstatement->execute();
				$parentrow = $this->getresult($profstatement); 		
				

				$shardobj = new Locker_Shard($currentShard,$shardHostName,$shardUserName,$shardPassword,$shardDB,$shardPort,$userrow[0]['total'],$parentrow[0]['total'],$assignmentrow[0]['total'],$groupsrow[0]['total'],$notecardrow[0]['total'],$citationrow[0]['total'],$taskrow[0]['total'],$savedassetrow[0]['total']);
				//Set Locker_Shard
				$retval[] = $shardobj;
				...
				{
					$currentShard = $shardrow[$i]['shard_id'];
					$shardHostName = $shardrow[$i]['db_hostname'];
					$shardUserName = $shardrow[$i]['db_username'];
					$shardPassword = $shardrow[$i]['db_password'];
					$shardDB = $shardrow[$i]['db_name'];
					$shardPort = $shardrow[$i]['db_port'];
					$profileid = $shardrow[$i]['profile_id'].'.'.$shardrow[$i]['parent_id'];
				}
				$queryEnd = '';
			}
	...
	*/
	...
	{			
		//Go through shards and create a userlist
		...
		for ($i = 0; $i<count($shardrow); $i++)
		{
			...
			//echo "$i Database Connection on locker1.".$shardrow[$i]['db_hostname'].' , '.$shardrow[$i]['db_username'].' , '.$shardrow[$i]['db_password'].' , '.$shardrow[$i]['db_name'].' , '. $shardrow[$i]['db_port']."<br>";			
			//Setup DB Connect
			$this->createLockerConnection($shardrow[$i]['db_hostname'], $shardrow[$i]['db_username'], $shardrow[$i]['db_password'], $shardrow[$i]['db_name'], $shardrow[$i]['db_port']);
			
			$profilestatement = $this->_lockerdbConn->prepare("CALL ".GET_EDUCATORPROFILE_LIST."()");
			$profilestatement->execute();	
			$profilerow = $this->getresult($profilestatement);
			
			for ($j = 0; $j<count($profilerow); $j++)
			{
				//AssignmentVars
				$emailaddress = NULL;
				$gradelist = NULL;
				...
				list($emailaddress, $gradelist, $interestlist, $marketingpreflist) = explode("|", $profilerow[$j]['profilevalue']);
				//CHECK TO SEE IF THEY OPT IN
				if(!empty($marketingpreflist))
				{
					$marketingprefarray = explode(",", $marketingpreflist);
				}
				...
				$profileobj = new Locker_Profile($profilerow[$j]['profileid'],'','',$profilerow[$j]['productid'], $profilerow[$j]['nickname'], $profilerow[$j]['securityquestionid'], $profilerow[$j]['securityanswer'], $profilerow[$j]['lastlogindate'],
							$profilerow[$j]['active'], $profilerow[$j]['creationdate'], $emailaddress,$profilerow[$j]['lexilerange'], $profilerow[$j]['profiletype'], 
							$profilerow[$j]['currentassignmentid'],$gradearray,$interestarray,$marketingprefarray);
				$retval[] = $profileobj;				
	...
	*/
	public function syncparentchildren()
	{
		$retval = -1;
		//GET ALL PARENTS 
		$statement = $this->_iauthDBConn->prepare("CALL ".GET_PARENTLIST."()");
		$statement->execute();	
		$parentlist = $this->getresult($statement);
...
//GET AUs for Parents
			$statement = $this->_iauthDBConn->prepare("CALL ".GET_AULIST."(?)");
			$statement->bind_param('s', $parentlist[$i]['parent_id']);
			$statement->execute();
			$parents_au_list = $this->getresult($statement);

//FOR EACH AUs
			for ($j = 0; $j<count($parents_au_list); $j++)
			{
//GET PREFS from parent Look in auth.au_pref
				$statement = $this->_iauthDBConn->prepare("CALL ".GET_PARENTPREFS."(?)");
				$statement->bind_param('i', $parents_au_list[$j]['au_id']);
				$statement->execute();
				$parent_prefs_row = $this->getresult($statement); 				
...
//GET CHILDREN 
			$statement = $this->_iauthDBConn->prepare("CALL ".GET_USERLISTBYPARENT."(?)");
			$statement->bind_param("s", $parentid);
			$statement->execute();	
			$userlist = $this->getresult($userliststatement);			
						
			for ($j = 0; $i<count($userlist); $j++)
			{					
				//Create Profile Object
				//$profileobj = new Locker_IAuthProfile('','','',$userlist[$i]['profile_id'].'.'.$parentid,$userlist[$i]['user_id'],$userlist[$i]['recipient_id'],$userlist[$i]['user_password'],$parentid);
			}
...
//INSERT PREFS/PRODS	
	...
	*/
	public function deletechildren($parentid)
	{
		...
		$statement->bind_param("s", $parentid);
		$statement->execute();	
		$userlist = $this->getresult($statement);			
		
		$retval = -1;
		
		for ($i = 0; $i<count($userlist); $i++)
		{	
			$profid = $userlist[$i]['profile_id'].".".$parentid;
//			echo $profid."|| ".$userlist[$i]['user_id']."<br>";
			$this->deleteuser($profid);
		}
	...
	*/
	...
	{
		list($tempprofid, $parentid) = split('[.]', $profileid);
		
		$profileid = $this->createDbConnection($profileid);	
	
		//Get list of assignments
		$statement = $this->_lockerdbConn->prepare("CALL ".GET_ALL_ASSIGNMENTS."(?)");
		$statement->bind_param("i", $profileid);
		$statement->execute();	
		$assignmentlist = $this->getresult($statement);
//echo"DEBUG: Assignment list<br>";
//print_r($assignmentlist);
		//Get list of tasks
		$statement = $this->_lockerdbConn->prepare("CALL ".GET_ALL_TASKS."(?)");
		$statement->bind_param("i", $profileid);
		$statement->execute();	
		$tasklist = $this->getresult($statement);
//echo"DEBUG: tasks list<br>";
//print_r($tasklist);		
		//Get list of outlines
		$statement = $this->_lockerdbConn->prepare("CALL ".GET_ALL_OUTLINES."(?)");
		$statement->bind_param("i", $profileid);
		$statement->execute();	
		$outlinelist = $this->getresult($statement);
//echo"DEBUG: outlines list<br>";
//print_r($outlinelist);				
		//Get list of citations
		$statement = $this->_lockerdbConn->prepare("CALL ".GET_ALL_CITATIONS."(?)");
		$statement->bind_param("i", $profileid);
		$statement->execute();	
		$citationlist = $this->getresult($statement);		
//echo"DEBUG: citations list<br>";
//print_r($citationlist);		
		//Get list of Biblios
		$statement = $this->_lockerdbConn->prepare("CALL ".GET_ALL_BIBLIOGRAPHIES."(?)");
		$statement->bind_param("i", $profileid);
		$statement->execute();	
		$bibliographylist = $this->getresult($statement);				
//echo"DEBUG: Biblios list<br>";
//print_r($bibliographylist);		

//echo"DEBUG: Delete User $profileid<br>";
		//Call remove user
		$statement = $this->_lockerdbConn->prepare("CALL ".DELETE_USER."(?)");
//echo"DEBUG: Delete User 1<br>";		
		$statement->bind_param("i", $profileid);
//echo"DEBUG: Delete User 2<br>";				
		$statement->execute();	
//echo"DEBUG: Delete User 3<br>";				

//echo"DEBUG: Delete User<br>";
//echo"DEBUG: Delete Ass Details <br>";
		//Remove from assignmentdetails
		for ($i = 0; $i<count($assignmentlist); $i++)
		{
			$statement = $this->_lockerdbConn->prepare("CALL ".DELETE_ASSIGNMENT_DETAILS."(?)");
			$statement->bind_param("s", $assignmentlist[$i]['assignmentid']);
			$statement->execute();	

//echo"DEBUG: Delete Ass Details For ::::: ".$assignmentlist[$i]['assignmentid']."<br>";
		}
//echo"DEBUG: Delete Task <br>";			
		//Remove from tasks
		for ($i = 0; $i<count($tasklist); $i++)
		{
			$statement = $this->_lockerdbConn->prepare("CALL ".DELETE_TASK."(?)");
			$statement->bind_param("s", $tasklist[$i]['taskid']);
			$statement->execute();	

//echo"DEBUG: Delete Task For ::::: ".$tasklist[$i]['taskid']."<br>";			
		}

//echo"DEBUG: Delete line <br>";			
		//Remove from line
		for ($i = 0; $i<count($outlinelist); $i++)
		{
			$statement = $this->_lockerdbConn->prepare("CALL ".DELETE_ALL_LINES."(?)");
			$statement->bind_param("s", $outlinelist[$i]['outlineid']);
			$statement->execute();	

//echo"DEBUG: Delete line For ::::: ".$outlinelist[$i]['outlineid']."<br>";						
		}		
		
//echo"DEBUG: Delete citation <br>";			
		//Remove from citation
		for ($i = 0; $i<count($citationlist); $i++)
		{
			$statement = $this->_lockerdbConn->prepare("CALL ".DELETE_CITATION."(?,?,?)");
			$statement->bind_param("iss", $citationlist[$i]['profileid'],$citationlist[$i]['assignmentid'],$citationlist[$i]['citationid']);
			$statement->execute();	

//echo"DEBUG: Delete citation For ::::: ".$citationlist[$i]['citationid']."<br>";									
		}		
		
//echo"DEBUG: Delete biblio <br>";			
		//Remove from biblio
		for ($i = 0; $i<count($bibliographylist); $i++)
		{
			$statement = $this->_lockerdbConn->prepare("CALL ".DELETE_BIBLIOGRAPHY."(?)");
			$statement->bind_param("s", $bibliographylist[$i]['bibliographyid']);
			$statement->execute();	

//echo"DEBUG: Delete biblio For ::::: ".$bibliographylist[$i]['bibliographyid']."<br>";						
		}		

//echo"DEBUG: LOCKER COMPLETE <br>";			

//echo"DEBUG: IAUTH START<br>";			
		//IAUTH
	
		//Get Customerid
		$statement = $this->_iauthDBConn->prepare("CALL ".GET_CUSTOMERID."(?)");
		$statement->bind_param("i", $profileid);
		$statement->execute();	
		$customerlist = $this->getresult($statement);
//echo"DEBUG: Customer list<br>";
//print_r($customerlist);

		//Remove auth_profile
		$statement = $this->_iauthDBConn->prepare("CALL ".DELETE_AUTHPROFILE."(?)");
		$statement->bind_param("i", $profileid);
		$statement->execute();	
//echo"DEBUG: Delete auth profile :::::: $profileid<br>";	

		//Remove customer
		$statement = $this->_iauthDBConn->prepare("CALL ".DELETE_CUSTOMER."(?)");
//ERROR CHECKING		
		$statement->bind_param("s", $customerlist[0]['customer_id']);
		$statement->execute();	
//echo"DEBUG: Delete customer :::::: ".$customerlist[0]['customer_id']."<br>";
		//Get Aus
		$statement = $this->_iauthDBConn->prepare("CALL ".GET_AUID."(?)");
//ERROR CHECKING
		$statement->bind_param("s", $customerlist[0]['customer_id']);
		$statement->execute();	
		$aulist = $this->getresult($statement);		
//echo"DEBUG: AU list<br>";
//print_r($aulist);

		//Get User_id
		$statement = $this->_iauthDBConn->prepare("CALL ".GET_AUUID."(?)");
//ERROR CHECKING
		$statement->bind_param("i", $aulist[0]['au_id']);
		$statement->execute();	
		$uidlist = $this->getresult($statement);
//echo"DEBUG: UID list<br>";
//print_r($uidlist);
				
		//Remove au
		$statement = $this->_iauthDBConn->prepare("CALL ".DELETE_AU."(?)");
//ERROR CHECKING		
		$statement->bind_param("s", $customerlist[0]['customer_id']);
		$statement->execute();	

//echo"DEBUG: Delete AU :::::: ".$customerlist[0]['customer_id']."<br>";
			
		//Remove au_uid
		$statement = $this->_iauthDBConn->prepare("CALL ".DELETE_AUUID."(?)");
//ERROR CHECKING		
		$statement->bind_param("i", $aulist[0]['au_id']);
		$statement->execute();	
		
//echo"DEBUG: Delete AUUID :::::: ".$aulist[0]['au_id']."<br>";		
		
		//Remove uid
		$statement = $this->_iauthDBConn->prepare("CALL ".DELETE_UID."(?)");
//ERROR CHECKING		
		$statement->bind_param("s", $uidlist[0]['user_id']);
		$statement->execute();	
		
//echo"DEBUG: Delete UID :::::: ".$uidlist[0]['user_id']."<br>";		
			
		//Remove au_pref
		$statement = $this->_iauthDBConn->prepare("CALL ".DELETE_AUPREF."(?)");
//ERROR CHECKING		
		$statement->bind_param("i", $aulist[0]['au_id']);
		$statement->execute();	
		
//echo"DEBUG: Delete AUPREF :::::: ".$aulist[0]['au_id']."<br>";		
				
		//Remove au_prod		
		$statement = $this->_iauthDBConn->prepare("CALL ".DELETE_AUPROD."(?)");
//ERROR CHECKING		
		$statement->bind_param("i", $aulist[0]['au_id']);
		$statement->execute();	
		
//echo"DEBUG: Delete AUPROF :::::: ".$aulist[0]['au_id']."<br>";				

		//Remove auth_sam
		$statement = $this->_iauthDBConn->prepare("CALL ".DELETE_AUTHSAM."(?)");
//ERROR CHECKING		
		$statement->bind_param("s", $customerlist[0]['customer_id']);
		$statement->execute();	
		
//echo"DEBUG: Delete AUTHSAM :::::: ".$customerlist[0]['customer_id']."<br>";		
				
		//Get Shardid

//echo"DEBUG: Lower shard count<br>";				
		if(!empty($parentid))
		{
			//Get Shard
			...
			$statement->execute();	
			$row = $this->getresult($statement);
//echo"DEBUG: Shard list<br>";
//print_r($row);			
			//Lower Shard count
			$shardtotal = $row[0]['total'] - 1;
			$statement = $this->_iauthDBConn->prepare("CALL ".UPDATE_SHARDCOUNT."(?, ?)");
			$statement->bind_param("ii", $row[0]['shard_id'], $shardtotal);
			$statement->execute();	
//echo"DEBUG: Lower count".$row[0]['shard_id']." -------> ".$shardtotal."<br>";
	...
	* @return 	string
	* 
	* @param	string $profilestring -  profileid + parentid
	*/	
...
*/		
	...
	/**
	* createLockerConnection
	* 
	* This function creates a locker connection based on some parameters.
	*
	* @author  	John Palmer
	* @access  	public 
	* 
	...
	* 
	*/	
	private function createLockerConnection($hostname, $username, $password, $database, $port)
	{
		//Close the old connectiont
    	if($this->_lockerdbConn != null)
    	{
		...
		$this->_lockerdbConn = new mysqli($hostname, $username, $password, $database, $port);

		if (mysqli_connect_errno()) 
		{
			throw new SoapFault("Server","Database Connection Error on locker1.".$hostname.' , '.$username.' , '.$password.' , '.$database.' , '.$port);
		}		
	...
	* executemultiquery
	*
	* This function takes in a query and returns a array of rows. NOT USED
	*
	* @author  	John Palmer
	* @access  	public 
	* 
...
echo "<pre>";
	...
	//print_r($server->getstuffcollected('22.iauth1003','22.21',NULL));
	//print_r($server->getlogininfo('jpalmer@scholastic.com', '274389'));
	//print_r($server->getprofile('1.6002370900'));
	
	//print_r($server->createSAMuser('718c1596-c326-42b8-8f75-ad3814e6b7a8', 'CT', $userdata));
	
	//print_r($server->validatelogin('samguidaupref11','samguidaupref11'));
	//print_r($server->validatelogin('johnp','validpass'));
	//print_r($server->deleteuser('2.6002370900'));
	print_r($server->deletechildren('6002370900'));
	
	//print_r($server->deleteuser('36.iauth1010','eto'));
