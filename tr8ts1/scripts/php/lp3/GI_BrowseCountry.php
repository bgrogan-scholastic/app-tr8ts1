...
Class BrowseCountry extends GI_Base {
	...
	function gen_countries($assetid) { 
		...
		<?php echo $current_country[0]['title_ascii']; ?></span>  </ul><p>
		<?php
		...
		<li><a href="/article/<?php echo $country_list[$i]['hier_cid']; ?>">
		<span class="countryList"><?php echo $country_list[$i]['country']; ?></span></a></li>
		<?php
		$i++;
	...
	function get() {
		 $output = $this->gen_countries($_GET['id']);
		 return $output;
	}
