...
class NewsUpdate:
	...
	self.movedfilelog = "/home/nbk3/newsadministration/moved_file_log.txt";
        ...
        if ( (not (cookies.has_key("saved-host") and cookies.has_key("username") and cookies.has_key("saved-pwd"))) and (host == "" and username=="" and password =="")):
	...
	print self.username, "<br>";
        ...
        try:
            #first, log in as the release user.
            #self.gits = giTelnet.giTelnet(self.host, "release", "36ptool");
            #now, switch to root.
            #presp = self.gits.suroot(self.password);            
	...
	print "<br>\n";
        ...
        response = self.gits.command("cd /export/home/plweb/");
        self.printList(response);               
        response = self.gits.command("clear");

        #create nbk3 news, restart plweb
        response = self.gits.command("./createfilelist.nbk3news", 60);
        self.printList(response);       
        response = self.gits.command("clear");
        
        response = self.gits.command("./create.nbk3news", 120);
        self.printList(response);       
        response = self.gits.command("clear");
