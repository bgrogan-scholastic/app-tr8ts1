	...
	$.fn.prettyPhoto = function(pp_settings) {
		...
		*/
		_showContent = function(){
			...
			$pp_pic_holder.find('.pp_content').animate({'height':correctSizes['contentHeight']+30},settings.animation_speed);
		...
		*/
		...
		function _buildOverlay(caller){
			...
			pp_descriptions = (isSet) ? jQuery.map(matchedObjects, function(n, i){ if($(n).attr('rel').indexOf(theRel) != -1) return ($(n).attr('rev')) ? $(n).attr('rev') : ""; }) : $.makeArray($(caller).attr('rev'));
