...
function scrollerCreate() {
  ...
  if (this.created) {
    // lah - 3/10/05 
    // added this b/c using the back button in ie/mac produces this following 
    // error
    if ( !(is_mac && is_ie) ){
      alert("Scroller Error: Scroller has already been created.");
      return;
  ...
  if (ie||dom) {
    ...
    // lah - 3/10/05 
    // adding in a specific check for ie4 so that this doesn't happen with ie5
    if (is_ie && ! is_ie5up) {
      if (!(ie&&window.print)) {
        x = getPageScrollX();
        y = getPageScrollY();
...
//EDIT HERE//////////////////////////////////////////////////////////////////////////////
  ...
  clipLayer(this.baseLayer, 0, 0, this.width, this.height);
   ...
   // Mac ie 5.2 is breaking on this line (red: 4/15/2005)
