/** 
	...
	@description: This file is used by the /assignment page
...
$(function() {
	// handles sorting of assets in list view
	bindClickSort();
});

/* 
	@author Nate Prouty
	@method displayCitationCount
	@params citationcount
	@result updates display of the citation count
	
*/
function displayCitationCount(citationcount) {
	//clear the citation count
	$('.sources_cited_count').html("");
	//if the citation count has not been returned try again
	if (($('.sources_cited_count').length == 0) || (typeof(citationcount) == 'undefined')) {
		setTimeout("displayCitationCount(citationcount)", 250);
	}
	//store the citation count
	citationcount = (citationcount == null) ? 0 : parseInt(citationcount);	
	//set the citation count
	if (citationcount == 1) {
		$('.sources_cited_count').html(citationcount + " Source Cited");
	}
	else {
		$('.sources_cited_count').html(citationcount + " Sources Cited");
	}
}

/* 
	@author Nate Prouty
	@method displayNoteCardCount
	@params notecardcount
	@result updates display of the note card count
	
*/

function displayNoteCardCount(notecardcount) {
	//clear the notecard count
	$('.notecard_count').html("");
	//if the note card count has not been passed try again
	if (($('.notecard_count').length == 0) || (typeof(notecardcount) == 'undefined')) {
		setTimeout("displayNoteCardCount(notecardcount)", 250);
	}
	//store the note card count
	notecardcount = (notecardcount == null) ? 0 : parseInt(notecardcount);	
	//set the note card count on the page
	if (notecardcount == 1) {
		$('.notecard_count').html(notecardcount + " Note Card");
	}
	else {
		$('.notecard_count').html(notecardcount + " Note Cards");
	}
}
...
/* 
	...
	@params json_stuffcollectedcounts
	@result displays the assets and their counts for the current assignment when in the folder view
*/
function displayAssetFolders(json_stuffcollectedcounts) {
	//reset the asset folders
	$('ul.assets').html("");
	//loop through all the collected assets
	$.each(json_stuffcollectedcounts, function(i, val) {
		var title = i;
		var count = val;
		...
		// Create the correct link, based on the type
		var label = '<a href="/assets?type=' + type + '" title="' + title + '">' + title + '</a>';
		
		var collected_item = '<li>' + label + '<span class="' + title + '_count"> (' + count + ') </span></li>';
		//rebuild the asset folder list
		$('ul.assets').append(collected_item);
	});
}

/* 
	@author Nate Prouty
	@method displayAssetList
	@params json_stuffcollected
	@result displays the list of assets for the current assignment when in the list view
*/
function displayAssetList(json_stuffcollected) {

	// don't continue until we have the json object
	...
	$.each(json_stuffcollected, function(i, val) {
		...
		var asset_type 		 = val._type;
						
		var output = "<tr>\n";	
		output += "<td class='article_title'>\n";
			
		// Media
		if ((asset_type == '0ma') || (asset_type == '0mp')) {
			output += "<a href='#' onclick='$(\"#mainImagePopup\").popup({contentLoadPath:\"" + asset_url + "\"}); return false;' title='" + asset_title + "'>" + asset_title + "</a>\n";
		}
		// Project Ideas
		else if (asset_type == '0taj') {
			output += "<p class='eduTxt'><a href='#' onclick=\"loadIdea('" + asset_major + "'); return false;\">" + asset_title + "</a></p>\n";
		}
		// Video
		else if (asset_type == '0uv') {
		...
		// Web Links
		else if (typeof(asset_type) == 'undefined') {
			output += '<a href="javascript:thePopup1.blurbWindow(\'' + asset_url + '\', 725, 600, \'gii\', \'on\');" title="'+ asset_title + '">' + asset_title + '</a>';	
		}
	...
	// sets the background color of every other row in the table
...
// The following functions handle the sorting of assets on the list view of the /assignment page
...
function sortTypeAsc(a,b) {
	var tmpa = getTypeName(a._digitallockerfoldertypeid); // see above
...
/*
	Called when the user clicks on a video asset on the list view of the /assignment page
	By Scholastic
*/
function playVideo(e) {
	...
	e.preventDefault();
	 ...
	 var str = loadAnchorVideo($(e.target).attr("href"),$(e.target).attr("title")); // see below
...
// Loads the requested video 	By Scholastic
...
function displayTaskList(json_tasklist) { 
	...
	$.each(json_tasklist, function(i, val) {
		...
		if ((tasktype != 1) && (tasktype != 6)) {
			// creates the self evaluation link
			var evaluationLink = "<a href='#' title='Answer questions about this task' onclick='getrubricquestionlist(" + tasktype + "," + taskid + ")'; return false;'>Self-evaluation</a>";
		}
...
function loadAssignmentList(json_assignmentlist) {
	...
	$(function() {
		$.each(json_assignmentlist, function(i, val) {
			// Add all but the current assignment to the drop down
